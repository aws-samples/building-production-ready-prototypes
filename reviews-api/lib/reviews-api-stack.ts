...
export class ReviewsApiStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const reviewsResources = apiConstruct.apiGateway.root.addResource("reviews");
    ...
    const lambdaToDynamoDBConstruct = new LambdaToDynamoDB(this, 'lambda-to-dynamo', {
    ...
    const lambdaDurationMetric = new cw.Metric({
      metricName: 'Duration',
          ...
          dimensionsMap: {'FunctionName': SQSToLambdaConstruct.lambdaFunction.functionName },
          statistic: 'p99',
    ...
    const lambdaDurationAlarm = lambdaDurationMetric.createAlarm(this, 'lambda-duration-alarm', {
      threshold: 500,
      evaluationPeriods: 1,
      alarmDescription: 'Alarm if P99 is over 500 ms'
    });
    const APIGatewayAlarm = ApiGatewayLatencyMetric.createAlarm(this, 'api-gateway-alarm', {
      threshold: 1000,
      evaluationPeriods: 1,
